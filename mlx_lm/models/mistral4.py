...
class Model(nn.Module):
    ...
    def sanitize(self, weights):
        ...
        # Dequantize MoE gate weights and remap to nn.Linear submodule
        remapped = {}
        for k, v in weights.items():
            if "rotary_emb.inv_freq" in k:
                continue
            if ".mlp.gate.scales" in k or ".mlp.gate.biases" in k:
                continue
            if ".mlp.gate.weight" in k:
                scales_key = k.replace(".weight", ".scales")
                biases_key = k.replace(".weight", ".biases")
                if scales_key in weights:
                    scales = weights[scales_key]
                    biases = weights[biases_key]
                    bits = (v.shape[-1] * 32) // self.args.hidden_size
                    group_size = self.args.hidden_size // scales.shape[-1]
                    v = mx.dequantize(v, scales, biases, bits=bits, group_size=group_size)
                remapped[k.replace(".mlp.gate.", ".mlp.gate.linear.")] = v
                continue
            remapped[k] = v
        return remapped
